...
describe('StringUtils', () => {
	describe('#contains', () => {
		...
		function testContains(string: string, phrase: string | number | boolean, expected = true) {
			const type = TypeUtils.getType(phrase)
			const desc = expected ? 'contains\t' : 'does not contain'

			// 'this is a string' contains 			'string' 	{string}
			// 'this is a string' does not contain 	'thing' 	{string}
			it(`'${string}'\t${desc}\t'${phrase}'\t{${type}}: `, () =>
				expect(StringUtils.contains(string, phrase)).toBe(expected))
		}
		...
		describe('throws an error if', () => {
			it(`'string' is undefined or null`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.capitalize(element as unknown as string)).toThrow()
				}
			})
		})
	})
	...
	describe('#capitalize', () => {
		...
		const THROWS = [1, function () {}, [], {}, new Object(), undefined, null]

		function testCapitalize(string: string, expected: string) {
			// 'string'
			it(`'${string}'`, () => expect(StringUtils.capitalize(string)).toBe(expected))
		}
	...
	describe('#toCamelCase', () => {
		...
		function testToCamelCase(string: string, expected: string, capitalize = false) {
			// 'a string' to 'aString'
			it(`'${string}'\tto\t'${expected}'`, () =>
				expect(StringUtils.toCamelCase(string, capitalize)).toBe(expected))
		}
		...
		describe('throws an error if', () => {
			it(`'string' is not a string`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.toCamelCase(element as unknown as string)).toThrow()
				}
			})
		})
	})
	...
	describe('#toKebabCase', () => {
		...
		function testToKebabCase(string: string, expected: string, capitalize = false) {
			...
			it(`'${string}'\tto\t'${expected}'`, () =>
				expect(StringUtils.toKebabCase(string, capitalize)).toBe(expected))
		}
		describe('converts', () => {
		...
		describe('throws an error if', () => {
			it(`'string' is not a string`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.toKebabCase(element as unknown as string)).toThrow()
				}
			})
		})
	})
	...
	describe('#toSnakeCase', () => {
		...
		function testToSnakeCase(string: string, expected: string, capitalize = false) {
			...
			it(`'${string}'\tto\t'${expected}'`, () =>
				expect(StringUtils.toSnakeCase(string, capitalize)).toBe(expected))
		}
		...
		describe('throws an error if', () => {
			it(`'string' is not a string`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.toSnakeCase(element as unknown as string)).toThrow()
				}
			})
		})
	})
	...
	describe('#toReadable', () => {
		...
		const THROWS = [1, function () {}, [], {}, new Object(), null]

		function testToReadable(string: string, expected: string, separator?: string | undefined) {
			// 'a_string' to 'a string'
			it(`'${string}'\tto\t'${expected}'`, () =>
				expect(StringUtils.toReadable(string, separator)).toBe(expected))
		}
		...
		describe('throws an error if', () => {
			it(`'string' is not a string`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.toReadable(element as unknown as string)).toThrow()
				}
			})

			it(`'separator' is not a string, (excluding undefined)`, () => {
				for (const element of THROWS) {
					expect(() => StringUtils.toReadable(TEST.COMMA, element as unknown as string)).toThrow()
				}
			})
		})
	})
	...
	describe('#toWordArray', () => {
		...
		function testToWordArray(string: string, expected: string[]) {
			let expectedStr = expected.reduce((str, element) => {
				return (str += `'${element}', `)
			}, '')
			...
			it(`'${string}'\tto\t'[${expectedStr}]' (of length '${expected.length}')`, () => {
				const test = StringUtils.toWordArray(string)
